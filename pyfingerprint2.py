...
class PyFingerprint(object):
    ...
    #         raise ValueError('File "' + imageLocation + '" does not exists!')

    #     maxPacketSize = self.getMaxPacketSize()

    #     print("Sending first packet to upload image...")

    #     packetPayload = (FINGERPRINT_UPLOADIMAGE, )

    #     self.__writePacket(FINGERPRINT_COMMANDPACKET, packetPayload)

    #     receivingPacket = self.__readPacket()

    #     receivedPacketType = receivingPacket[0]
    #     receivedPacketType = receivingPacket[1]

    #     if receivedPacketType != FINGERPRINT_ACKPACKET:
    #         raise Exception('The received packet is not ack packet!')

    #     if receivedPacketType[0] == FINGERPRINT_OK:
    #         pass
    #     elif receivedPacketType[0] == FINGERPRINT_ERROR_COMMUNICATION:
    #         raise Exception('Communication Error')
    #     elif receivedPacketType[0] == FINGERPRINT_UPLOADIMAGE:
    #         raise Exception('Could not upload the Image')
    #     else:
    #         raise Exception('Unkown error ' + hex(receivedPacketType[0]))
    ...
    #         print("sending 1 row of pixels at a time")
    #         i = 1
    ...
    #     else:
    #         print("flattening pixel' arrays into a list and sending packets with length: ") + str(maxPacketSize)
        
    #     flattenedList = list(imageArray.flatten())
    #     packetNbr = len(flattenedList)
        
    #     if packetNbr <= 1:
    #         print("Only one packet to send. Sending...")
    #         self.__writePacket(FINGERPRINT_ENDDATAPACKET, flattenedList)
        
    #     else:
    #         print("Sending " + str(packetNbr) + " packets. This may take a while ....")
    #         i = 1
    #         while i < packetNbr:
    ...
    def uploadCharacteristics(self, charBufferNumber = 0x01, characteristicsData = [0]):
        ...
        if ( packetNbr <= 1 ):
            ...
            while ( i < packetNbr ):
                ...
                # print(characteristicsData[lfrom:lto])
        ...
        l = len(characterics)
        form = 'B' * l
        print(binascii.hexlify(struct.pack(form, *characterics)))
    ...
    def downloadCharacteristics(self, charBufferNumber = 0x01):
        ...
        while ( receivedPacketType != FINGERPRINT_ENDDATAPACKET ):
            ...
            lenCompletePacketPayload += len(receivedPacketPayload)
            # print(len(receivedPacketPayload))
            # form = 'B' * len(receivedPacketPayload)
            # print(binascii.hexlify(struct.pack(form, *completePayload)))
        # print(lenCompletePacketPayload)
        # form = 'B' * lenCompletePacketPayload
        # print(binascii.hexlify(struct.pack(form, *completePayload)))
